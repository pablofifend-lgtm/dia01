...
var getElement = function (store, key) {
    var open = indexedDB.open('minerales');
    return new Promise(function (resolve, reject) {
        open.onsuccess = function () {
            var request;
            db = open.result;
            if (__spreadArray([], db.objectStoreNames, true).find(function (name) { return name === store; })) {
                ...
                indexedDB.deleteDatabase('minerals');
            }
        };
    });
};
...
var addElement = function (store, payload) {
    var open = indexedDB.open('minerals');
    open.onsuccess = function () {
        db = open.result;
        if (__spreadArray([], db.objectStoreNames, true).find(function (name) { return name === store; })) {
